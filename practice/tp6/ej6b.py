...
def ejercicio_6b(N):
    ...
    # 2. Generar todas las N muestras bootstrap
    muestras_bootstrap = calular_muestras_bootstrap(datos_originales, N)

    # 3. Calcular S^2 para cada muestra bootstrap
    replicaciones_s2 = [calcular_varianza_muestral(m) for m in muestras_bootstrap]
    ...
    varianza_bootstrap_final = suma_errores_cuadrados / (N - 1)

    return varianza_bootstrap_final


# Resultado esperado: 1.0
print(f"La estimación bootstrap de Var(S^2) es: {ejercicio_6b(10000)}")
